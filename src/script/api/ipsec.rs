...
// The runtime lookups now live in `crate::ipsec::runtime` — they are Rust-side
// state, not part of the Python surface. Re-exported here so the paths scripts
// and the dispatcher already use keep resolving.
pub(crate) use crate::ipsec::runtime::outbound_for_sa;
pub use crate::ipsec::runtime::{
    find_sa_for_ue, is_protected_local_port, outbound_for, outbound_local_addr_for,
    pcscf_path_host, record_route_port_for, repin_sa_for_ue,
};
...
impl PyIpsec {
    pub fn new(
        ...
        crate::ipsec::runtime::install(Arc::clone(&manager), Arc::clone(&config));
        Self {
            manager,
            config,
            pcscf_addr_v4,
