...
use shutdown_sequence::run_shutdown_sequence;
mod event_clock;
mod sbi_callbacks;
...
impl SiphonServer {
    ...
    async fn run_async(mut self) {
        ...
        // Wait for shutdown signal (SIGINT or SIGTERM), then drain, end what
        // the deadline is still holding, and exit.
        run_shutdown_sequence(
            &config,
            &drain,
            shutdown_registrant.as_ref(),
            &shutdown_outbound,
            dispatcher_handle,
        )
        .await;
