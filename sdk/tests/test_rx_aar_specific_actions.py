...
def test_default_requests_nothing_and_keeps_the_answer_shape(diameter):
    result = run(diameter.rx_aar(framed_ip="192.0.2.1"))
...
def test_every_defined_value_is_accepted(diameter, value):
    assert run(diameter.rx_aar(specific_actions=[value]))["result_code"] == 2001


def test_several_values_and_an_empty_list(diameter):
    assert run(diameter.rx_aar(specific_actions=[2, 6, 9])) is not None
    assert run(diameter.rx_aar(specific_actions=[])) is not None
    assert run(diameter.rx_aar(specific_actions=(7, 8))) is not None
...
def test_value_past_64_bits_raises_overflow_error(diameter):
    ...
    with pytest.raises(OverflowError):
        run(diameter.rx_aar(specific_actions=[2**70]))
