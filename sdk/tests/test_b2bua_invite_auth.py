...
def test_require_proxy_digest_on_call_arms_a_407_reject():
    ...
    assert run(auth.require_proxy_digest(call, realm="example.com")) is False
...
def test_require_www_digest_on_call_arms_a_401_reject():
    ...
    assert run(auth.require_www_digest(call, realm="example.com")) is False
    assert call.last_action.kind == "reject"
...
def test_require_digest_alias_on_call_arms_a_401_reject():
    ...
    assert run(auth.require_digest(call, realm="example.com")) is False
    assert call.last_action.status_code == 401
...
def test_authenticated_call_sets_auth_user_and_arms_nothing():
    ...
    assert run(auth.require_proxy_digest(call, realm="example.com")) is True
...
def test_authenticated_call_takes_username_from_the_credentials():
    ...
    assert run(auth.require_proxy_digest(fresh, realm="example.com")) is True
...
def test_verify_digest_on_call_reads_proxy_authorization():
    ...
    assert run(auth.verify_digest(without, realm="example.com")) is False

    with_credentials = _invite_call(
        proxy_auth='Digest username="alice", realm="example.com", nonce="abc"'
    )
    assert run(auth.verify_digest(with_credentials, realm="example.com")) is True
...
def test_request_path_is_unchanged():
    ...
    assert run(auth.require_proxy_digest(request, realm="example.com")) is False
...
def test_challenging_an_unsupported_object_raises_type_error():
    ...
    with pytest.raises(TypeError, match="Request .* or a Call"):
        run(auth.require_proxy_digest(NotAMessage(), realm="example.com"))
...
def test_b2bua_challenge_then_dial_flow():
    ...
    def on_invite(call):
        if not run(auth.require_proxy_digest(call, realm="example.com")):
            return
        call.dial(call.ruri)
